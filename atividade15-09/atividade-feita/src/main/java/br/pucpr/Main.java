...
public class Main {
  public static void main(String[] args)
    ...
    var tabelaUsuarios = new Table(dadosUsuarios, LIGHT, true);
    ...
    System.out.println("PLANETAS PAGINADOS (3 por pagina)");
    System.out.println("----------------------------------");
    final var planetasPaginados =
        new PagedTableData(new ColumnTableData<>(planetas, PlanetaColumns.values()), 3);
    for (int i = 0; i < planetasPaginados.getPageCount(); i++) {
      planetasPaginados.setPage(i);
      System.out.println("Pagina " + (i + 1) + " de " + planetasPaginados.getPageCount());
      new Table(planetasPaginados, LIGHT).print();
    }
  }
}
